...
Write-Host "`n🏥 Health & Session Tools:" -ForegroundColor Magenta
...
Write-Host "`n🔍 RAG & Knowledge Base Tools:" -ForegroundColor Magenta
...
Write-Host "`n📊 Project Management Tools:" -ForegroundColor Magenta
...
if ($results.passed -eq $results.total) {
    Write-Host "`n🎉 ALL TESTS PASSED! Ollama-powered Archon MCP is fully operational!" -ForegroundColor Green
} else {
    Write-Host "`n⚠️  $($results.failed) tests failed. Check details above." -ForegroundColor Yellow
}
...
Write-Host "`n📊 Tool Categories Status:" -ForegroundColor Cyan
