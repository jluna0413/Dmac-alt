...
Show JSON for scripting:
    python mcp_config.py show --json
"""
...
class MCPConfigManager:
    ...
    @staticmethod
    def _resolve_config_path(cli_path: Optional[str]) -> Path:
        """Resolve config path from CLI, env, or standard locations.

        Priority:
          1) CLI --config/-c
          2) MCP_CONFIG_FILE env var
          3) ./mcp_config.json
          4) %APPDATA%/mcp/mcp_config.json (Windows) or ~/.config/mcp/mcp_config.json (POSIX)
        """
        if cli_path:
        ...
        cwd_default = Path("mcp_config.json")
        ...
        if os.name == "nt":
            ...
            if appdata:
                candidate = Path(appdata) / "mcp" / "mcp_config.json"
                if candidate.exists():
                    return candidate
                return candidate  # return standard location even if it doesn't exist yet
        else:
            candidate = Path.home() / ".config" / "mcp" / "mcp_config.json"
            if candidate.exists():
                return candidate
            return candidate
        return cwd_default
    ...
    def _icons() -> Dict[str, str]:
        enc_raw: str = str(sys.stdout.encoding or locale.getpreferredencoding(False) or "")
        enc = enc_raw.lower()
        if "utf" in enc:
            return {
                "ok": "✅",
    ...
    def _validate_server_config(self) -> List[str]:
        ...
        if not url or not isinstance(url, str):
            ...
            if parsed.scheme not in {"http", "https"} or not parsed.netloc:
                issues.append("MCP server URL must be http(s)://host[:port]/mcp")
    ...
    def test_connection(self) -> bool:
        ...
        icons = self._icons()
        print(f"{icons['link']} Testing connection to {url} (attempts={attempts}, delay={delay}s)")

        base_url = url.replace("/mcp", "") if url else url
        for i in range(1, attempts + 1):
            try:
                resp = requests.get(base_url, timeout=timeout)
                print(f"{icons['ok']} Base endpoint reachable - HTTP {resp.status_code}")
                ...
                mcp_resp = requests.post(url, json=mcp_request, headers=headers, timeout=timeout)
                if mcp_resp.status_code == 200:
                    print(f"{icons['ok']} MCP protocol response - HTTP {mcp_resp.status_code}")
                    return True
                warn_left = attempts - i
                print(f"{icons['warn']} MCP endpoint returned HTTP {mcp_resp.status_code} (retries left: {warn_left})")
            except requests.RequestException as e:
                warn_left = attempts - i
                print(f"{icons['warn']} Connection error: {e} (retries left: {warn_left})")
    ...
    def show_config(self) -> None:
        ...
        print(f"  • Config file: {self.config_file} ({'exists' if self.config_file.exists() else 'not found'})")
...
def _create_argument_parser() -> argparse.ArgumentParser:
    ...
    show_cmd = sub.add_parser("show", help="Show current configuration")
    show_cmd.add_argument("--json", action="store_true", help="Output raw JSON configuration")
    ...
    gen.add_argument("--force", "-f", action="store_true", help="Overwrite without prompt")
